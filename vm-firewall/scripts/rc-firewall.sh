...
# VNIC LAN 
vnic_lan_ip="`curl -s -H "Authorization: Bearer Oracle" -L http://169.254.169.254/opc/v2/instance/metadata/vnic-lan-ip`"
vnic_lan_ip_gw="`curl -s -H "Authorization: Bearer Oracle" -L http://169.254.169.254/opc/v2/instance/metadata/vnic-lan-ip-gw`"
vnic_lan_iface="`ip -o -f inet addr show | grep "$vnic_lan_ip" | awk '{print $2}'`"
...
ip rule add from $vnic_lan_ip fwmark 3000 lookup internet priority 3000
...
iptables -t mangle -A OUTPUT -o $vnic_lan_iface -s $vnic_lan_ip -d 169.254.0.0/16 -j RETURN

# NAT para a INTERNET
iptables -t nat -A POSTROUTING -o $vnic_internet_iface -j MASQUERADE

# Permite que o firewall tenha acesso à Internet.
iptables -t mangle -A OUTPUT -o $vnic_lan_iface -s $vnic_lan_ip -d 0.0.0.0/0 -j MARK --set-mark 3000
iptables -t mangle -A OUTPUT -o $vnic_lan_iface -s $vnic_lan_ip -d 0.0.0.0/0 -j RETURN
...
iptables -t mangle -A PREROUTING -i $vnic_lan_iface -s $vcn_appl_1_cidr -d 0.0.0.0/0 -j MARK --set-mark 3050  
iptables -t mangle -A PREROUTING -i $vnic_lan_iface -s $vcn_appl_1_cidr -d 0.0.0.0/0 -j RETURN
iptables -t mangle -A PREROUTING -i $vnic_lan_iface -s $vcn_appl_2_cidr -d 0.0.0.0/0 -j MARK --set-mark 3050  
iptables -t mangle -A PREROUTING -i $vnic_lan_iface -s $vcn_appl_2_cidr -d 0.0.0.0/0 -j RETURN
